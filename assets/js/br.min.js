...
function kontaktSkripta(){
    ...
    jQuery('body').on('click', '#btnContactUs', function(e){
        ...
        var $rezultat = jQuery('input[name=recaptcha]').val();
        if ($rezultatab != $rezultat) {
                jQuery(".kaptcha input").css('border-color','red');
                proceed = false;      
        }                   
        if(proceed) {
            post_data = {
            ...
            jQuery.post('contact_me.php', post_data, function(response){  
                ...
                jQuery("#sendmessage").hide().html(output).slideDown();
            }, 'json');
        }
    });
    jQuery("#contact-form  input, #contact-form textarea").keyup(function() {
        jQuery(this).css('border-color','');
        jQuery("#sendmessage").slideUp();
    });
}

$(function(){
	onReadySkripte();
	if ($('.kontakt_forma_button').length && kontaktPostoji === false) {
		kontaktSkripta();
	}
	'use strict';
	var $page = $('#sajt_content'),
	options = {
		debug: true,
		prefetch: true,
		cacheLength: 7,
